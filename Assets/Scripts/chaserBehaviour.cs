...
public class chaserBehaviour : MonoBehaviour
{
    ...
    void FixedUpdate()                               // CHANGED from Update()
    {
        ...
        if (((Vector2)_rb.position - target).sqrMagnitude > leashDistance * leashDistance)
        {
            ...
            {
                if (worldState.instance != null && worldState.instance.aggressiveBlocking && Random.value < interceptChance)
                    _rb.position = EnemyPathing.ComputeInterceptPoint(_rb.position, target, worldState.instance.playerVelocity, Camera.main, leashOffscreenPad, interceptJitter);
                else
        ...
        // Intercept-leash: when aggressiveBlocking is on and player moving away, probabilistically teleport to intercept
        if (worldState.instance != null && worldState.instance.aggressiveBlocking)
        {
            ...
            if (Vector2.Dot(toPlayer, playerVel) > 0 && Random.value < interceptChance)
            {
