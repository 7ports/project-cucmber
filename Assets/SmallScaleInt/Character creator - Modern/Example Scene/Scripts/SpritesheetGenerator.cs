...
private Color[] ResizePixelsPixelPerfect(Color[] original, int origWidth, int origHeight, int newWidth, int newHeight)
{
    ...
    float xRatio = origWidth / (float)newWidth;
    float yRatio = origHeight / (float)newHeight;
    for (int y = 0; y < newHeight; y++)
    {
        for (int x = 0; x < newWidth; x++)
        {
            int origX = Mathf.FloorToInt(x * xRatio);
            int origY = Mathf.FloorToInt(y * yRatio);
            if (origX < 0) origX = 0;
