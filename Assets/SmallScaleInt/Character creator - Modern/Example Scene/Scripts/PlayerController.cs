...
namespace SmallScaleInc.CharacterCreatorModern
{
    public class PlayerController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            Vector2 mousePosition = Camera.main.ScreenToWorldPoint(GetMouseScreenPosition());
            ...
            bool isMoving = IsKeyPressed(Key.W) || IsKeyPressed(Key.A) ||
                             IsKeyPressed(Key.S) || IsKeyPressed(Key.D);
            ...
            if (isActive)
            {
                ...
                if (isRanged)
                {
                    ...
                    if (WasKeyPressedThisFrame(Key.Digit1))
                    {
                        StartCoroutine(DeploySpecial1Delayed());
                    }
                    if (WasKeyPressedThisFrame(Key.Digit3))
                    {
                        StartCoroutine(DeployAoEDelayed());
                    }
                    if (WasKeyPressedThisFrame(Key.Digit5))
                    {
                        if (isSummoner)
                        {
                    ...
                    if (WasKeyPressedThisFrame(Key.Digit6))
                    {
                ...
                else if (WasKeyPressedThisFrame(Key.LeftCtrl) && isRunning)
                {
                    if (isShapeShifter && isActive)
                    {
                        StartCoroutine(ShapeShiftDelayed());
        ...
        {
            if (WasRightMousePressedThisFrame()) // Right mouse button pressed
            {
        ...
        private IEnumerator StopGunfireSoundAfterDelay()
        {
            ...
            if (!IsRightMousePressed()) // Check if the button is still being held
            {
