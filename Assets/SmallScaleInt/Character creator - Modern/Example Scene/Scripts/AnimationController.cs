...
namespace SmallScaleInc.CharacterCreatorModern
{
    public class AnimationController : MonoBehaviour
    {
        ...
        private static bool WasRightMouseReleasedThisFrame()
        {
            var mouse = Mouse.current;
        ...
        void Update()
        {
            ...
            else if (IsKeyPressed(Key.Digit1))
            {
                TriggerTakeDamageAnimation();
            }
            else if (IsKeyPressed(Key.Digit2))
            {
                TriggerSpecialAbility2Animation();
            }
            else if (IsKeyPressed(Key.Digit3))
            {
                TriggerCastSpellAnimation();
            }
            else if (IsKeyPressed(Key.Digit4))
            {
                TriggerKickAnimation();
            }
            else if (IsKeyPressed(Key.Digit5))
            {
                TriggerPummelAnimation();
            }
            else if (IsKeyPressed(Key.Digit6))
            {
                TriggerAttackSpinAnimation();
            }
            else if (IsKeyPressed(Key.Digit7))
            {
                TriggerDie();
            }
            else if (IsKeyPressed(Key.LeftShift) && isCurrentlyRunning)
            {
                TriggerFlipAnimation();
            }
            else if (IsKeyPressed(Key.LeftCtrl) && isCurrentlyRunning)
            {
                TriggerRollAnimation();
            }
            else if (IsKeyPressed(Key.LeftAlt) && isCurrentlyRunning)
            {
        ...
        void HandleMovement()
        {
            ...
            isRunning = IsKeyPressed(Key.W);
            isRunningBackwards = IsKeyPressed(Key.S);
            isStrafingLeft = IsKeyPressed(Key.A);
            isStrafingRight = IsKeyPressed(Key.D);
        ...
        void HandleAttackAttack()
        {
            ...
            else if (WasRightMouseReleasedThisFrame())
            {
